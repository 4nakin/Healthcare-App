...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        int orientation = a.getInt(R.styleable.MySlider_orientation, ORIENTATION_VERTICAL);
        mVertical = orientation == ORIENTATION_VERTICAL;
        mBottomOffset = (int) a.getDimension(R.styleable.MySlider_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.MySlider_topOffset, 0.0f);
        mAllowSingleTap = a.getBoolean(R.styleable.MySlider_allowSingleTap, true);
        mAnimateOnClick = a.getBoolean(R.styleable.MySlider_animateOnClick, true);

        int handleId = a.getResourceId(R.styleable.MySlider_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.MySlider_content, 0);
        if (contentId == 0) {
